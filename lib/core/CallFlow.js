			...
			if (status == 'completed' || status == 'empty') {
				...
				return cf._currState._onExit.execute(cf, cf._currState, event, data).then(function () {
					...
					if (nextState) {
						...
						return cf._currState._onEntry.execute(cf, cf._currState, event, data).then(function (eventObj) {
							if (eventObj.stopped) {
								return;
							}
					...
					return cf._currState._onExit.execute(cf, cf._currState, event, data).then(function () {
						...
						if (statesStack) {
							...
							if (lastStateInStack &&
								...
								return lastStateInStack._parentCallFlow.fireEvent(event, data, statesStack);
							}
						}
					});
				}
